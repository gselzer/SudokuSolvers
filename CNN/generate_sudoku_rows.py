#!/usr/bin/python2
"""
Generates a given number of Sudoku puzzles, where a random number of
rows at the bottom are missing.
...
def pluck(puzzle, n=0):
    for i in range(n, 9):
        puzzle[i] = [0 for x in range(len(puzzle[i]))]
        print('puzzle = ', puzzle)

    # This is the puzzle we found, in all its glory.
    return (puzzle, 81 - (9 * n))
